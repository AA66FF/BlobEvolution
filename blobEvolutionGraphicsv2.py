...
def mutate(speed,aggro,aggRange,size,attack,attackRange,mHealth,color):
    ...
    if r == 6:
        ...
        if rr == 2:
            a = not a
        return [speed+uniform(-0.003*mm,0.003*mm),a,aggRange+\
        uniform(-1*mm,1*mm),size+uniform(-0.03*mm,0.03*mm),\
        attack+uniform(-4.5*mm,4.5*mm),attackRange+uniform(-1*mm,\
        1*mm),mHealth+uniform(-6*mm,6*mm),c]
    else:
        a = aggro
        rr = randint(0,8)
        if rr == 8:
            a = not a
        return [speed+uniform(-0.001*mm,0.001*mm),a,aggRange+\
...
class BlobCircle(Sprite):
    def __init__(self, rad, fill, outline):
        self.image = Circle(Point(convToPoint(self.pos)),rad)
        self.image.setFill = fill
        self.image.setOutline = outline

    def draw(self):
        image = self.image
        image.p1 = Point(self.pos[0]-rad,self.pos[1]-rad)
        image.p2 = Point(self.pos[0]+rad,self.pos[1]+rad)
        image.draw(win)
...
class attackLine(Sprite):
    def __init__(self, pos2):
        self.pos2 = [pos2[0],pos2[1]]
        self.image = Line(convToPoint(self.pos),convToPoint(pos2))
        self.image.setFill("#440000")
        self.image.setOutline("#440000")

    def draw(self):
        image = self.image
        image.p1 = convToPoint(self.pos)
        image.p2 = convToPoint(self.pos2)
        image.draw(win)
...
class Bar(Sprite):
    def __init__(self, pos2, back1, back2, fill, fill2):
        self.pos2 = pos2
        self.back1 = back1
        self.back2 = back2
        self.image = Rectangle(convToPoint(self.pos),convToPoint(pos2))
        self.back = Rectangle(convToPoint(back1),convToPoint(back2))
        self.image.setFill(fill)
        self.image.setOutline(fill)
        self.back.setFill(fill2)
        self.back.setOutline(fill2)
    ...
    def draw(self):
        ...
        image.p1 = convToPoint(self.pos)
        image.p2 = convToPoint(self.pos2)
        back.p1 = convToPoint(self.back)
        back.p2 = convToPoint(self.back2)
        image.draw(win)
        back.draw(win)
...
while True:
    ...
    for i in range(len(blobs)-1, -1, -1):
        ...
        blobs[blobs[i].target].age > immunityTime:
            ...
            blobs[blobs[i].target].health -= \
            blobs[i].effAtt*(blobs[i].size**sizeHealthBuff)
        blobs[i].update()
